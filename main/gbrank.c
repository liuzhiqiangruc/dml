...
void help() {
    fprintf(stderr, "\ngbrank usage:        \n");
    fprintf(stderr, "\n./gbrank -n <int> -p <int> -m <int> -d <int> -l <int> -h <double> -b <int> -a <double> -g <double> -r <double> -f <string> -s <string> -t <string> -y <string> -o <string>\n");
    ...
    fprintf(stderr, "     -h  max margin for hinge loss       \n");
...
int main(int argc, char *argv[]) {
    ...
    GBDT * gbrank = gbdt_rank(p);
    if (!gbrank){
        return -1;
    }
    fprintf(stderr, "train rows : %d", y_rowns(gbrank));
    if (1 == has_test(gbrank)){
        fprintf(stderr, ", test rows : %d", t_rowns(gbrank));
    }
