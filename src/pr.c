...
void help() {
    fprintf(stderr, "\nPR [Possion Regression] usage:        \n");
    fprintf(stderr, "\n./pr -a <double> -l <double> -b <int> -r <int> -n <int> -s <int> -f <string> -t <string> -o <string\n");
...
int main(int argc, char *argv[]) {
    PR *pr = create_regression();
    if (-1 == parse_command_line(&(pr->p), argc, argv)){
        help();
        goto except;
    }
    fprintf(stderr, "command line parse done\n");
    if (-1 == init_regression(pr)){
        goto except;
    }
    ...
    learn_regression(pr);
    save_regression(pr, pr->p.niters);
    free_regression(pr);
    pr = NULL;
    return 0;
