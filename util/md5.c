...
void MD5_Update(MD5Context * context, unsigned char * buf, int len){
    ...
    if (len >= partLen){
        memmove((char *) &context->buffer[index], (char *) buf, partLen);
    ...
    memmove((char *) &context->buffer[index], (char *) &buf[i], len - i);
}
...
int gen_md5_digest(const char * instr, unsigned char out_digest[16]){
    if (!instr || strlen(instr) == 0)
        return -1;
    int len = strlen(instr);
    memset(out_digest,0,16);
    MD5_Init(&context);
    MD5_Update(&context,(unsigned char *)instr, len);
    MD5_Final(&context,out_digest);
    return 0;
}

/*
int main(){
    const char * a[2]  = {"abcdefghijklmnopqrstuvwxyz","abc"};
    unsigned char digest[16];
    for(int j = 0; j< 2; j++){
        gen_md5_digest(a[j],digest);
        for (int i = 0; i< 16; i++){
            printf("%x",digest[i] >>4);
