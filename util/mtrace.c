 ...
 *   date     : 2014-12-31
 *   info     : implementation of median value trace
 * ======================================================== */
...
/* ----------------------------------------
 * Int vector median trace by default
 * ---------------------------------------- */
...
static void m_insert_left(MTrace * m, void * pdata){
    ...
    m->cnt_1 += 1;
    if ((m->left_max && m->cmp_fn(pdata, m->left_max) > 0) || \
        (!m->left_max)){
        m->left_max = pdata;
    }
}
...
static void m_insert_right(MTrace * m, void * pdata){
    ...
    m->cnt_2 += 1;
    if ((m->right_min && m->cmp_fn(pdata, m->right_min) < 0) || \
        (!m->right_min)){
        m->right_min = pdata;
    }
}
...
void m_add(MTrace * m, void *pdata){
    if (m->cnt_1 == 0 || m->cmp_fn(pdata, m->left_max) > 0){
